...
func TestSessionParseFile(t *testing.T) {
	...
	if !ok || s.HasErrors() { t.Fatalf("parse failed: ok=%v diagnostics=%v", ok, s.Diagnostics.All()) }
	if len(file.Statements) != 1 { t.Fatalf("got %d statements", len(file.Statements)) }
	if _, ok := file.Statements[0].(ast.VariableDecl); !ok { t.Fatalf("unexpected statement: %#v", file.Statements[0]) }
}

func TestSessionParseMissingFile(t *testing.T) {
	s := NewSession()
	_, ok := s.ParseFile(999)
	if ok || !s.HasErrors() { t.Fatal("expected missing-file diagnostic") }
}
