...
func (l *Lexer) number(start int) token.Token {
	if l.cursor.Match("0x") || l.cursor.Match("0X") {
		return l.radixNumber(start, 16)
	}
	if l.cursor.Match("0b") || l.cursor.Match("0B") {
		return l.radixNumber(start, 2)
	}
	if l.cursor.Match("0o") || l.cursor.Match("0O") {
		return l.radixNumber(start, 8)
	}
	if !l.consumeDigits(10) {
		return l.invalidNumber(start, "This number is missing digits.", "Write a number such as 42.")
	}
	if !l.cursor.Done() {
		r, _ := l.cursor.Peek()
		if r == '.' {
	...
	if l.hasInvalidNumericContinuation() {
		l.consumeIdentifierContinuation()
		return l.invalidNumber(start, "This number has an invalid trailing character.", "Separate a number from an identifier or use a valid numeric suffix.")
	}
	return l.make(token.Number, start)
}

func (l *Lexer) radixNumber(start, base int) token.Token {
	if !l.consumeDigits(base) {
		return l.invalidNumber(start, "This based number is missing valid digits.", "Use valid digits after the hexadecimal, binary, or octal prefix.")
	}
	if !l.cursor.Done() {
		r, _ := l.cursor.Peek()
		if r == 'n' {
...
func (l *Lexer) hasInvalidNumericContinuation() bool {
	if l.cursor.Done() {
		return false
	}
	r, _ := l.cursor.Peek()
...
func (l *Lexer) consumeIdentifierContinuation() {
	for !l.cursor.Done() {
		r, _ := l.cursor.Peek()
		if !isIdentifierContinue(r) {
			break
...
func (l *Lexer) consumeHexEscape(n int) bool {
	...
	for i := 0; i < n; i++ {
		...
		r, _ := l.cursor.Peek()
		if digitValue(r) < 0 || digitValue(r) >= 16 {
			return false
		}
		l.advanceRune()
	}
...
func (l *Lexer) consumeUnicodeEscape() bool {
	...
	if r == '{' {
		...
		for !l.cursor.Done() {
			...
			if r == '}' {
				l.advanceRune()
				return digits > 0 && digits <= 6 && value <= 0x10FFFF && !(value >= 0xD800 && value <= 0xDFFF)
			}
			d := digitValue(r)
			if d < 0 || d >= 16 {
				return false
...
func (l *Lexer) invalid(start int, message, hint string) token.Token {
	l.diagnostics.Add(diagnostics.Diagnostic{Severity: diagnostics.Error, Code: "G1000", Message: message, Hint: hint, Span: l.span(start)})
	return l.make(token.Invalid, start)
}
func (l *Lexer) invalidNumber(start int, message, hint string) token.Token {
	l.diagnostics.Add(diagnostics.Diagnostic{Severity: diagnostics.Error, Code: "G1002", Message: message, Hint: hint, Span: l.span(start)})
	return l.make(token.Invalid, start)
}
