...
func TestParseVariableDeclaration(t *testing.T) {
	...
	if !ok || decl.Name.Name != "user" {
		t.Fatalf("unexpected declaration: %#v", file.Statements[0])
	}
}
...
func TestParseExpressionPrecedence(t *testing.T) {
	...
	if !ok || root.Operator != "+" {
		t.Fatalf("unexpected root: %#v", expr)
	}
	if _, ok := root.Right.(ast.BinaryExpr); !ok {
		t.Fatalf("expected multiplication on right: %#v", root.Right)
	}
}
...
func TestParseCall(t *testing.T) {
	...
	if !ok || len(call.Arguments) != 2 {
		t.Fatalf("unexpected call: %#v", file.Statements[0])
	}
}
...
func TestParseMissingBraceDiagnostic(t *testing.T) {
	...
	if len(p.Diagnostics()) == 0 || p.Diagnostics()[0].Code != "G2006" {
		t.Fatalf("expected G2006, got %v", p.Diagnostics())
	}
}

func TestParseMissingValueDiagnostic(t *testing.T) {
	p, _ := parse(`create variable user as`)
	if len(p.Diagnostics()) == 0 || p.Diagnostics()[0].Code != "G2004" {
		t.Fatalf("expected G2004, got %v", p.Diagnostics())
	}
}
...
func TestParserNeverUsesInvalidTokensSilently(t *testing.T) {
	...
	p.ParseFile()
	if len(p.Diagnostics()) == 0 {
		t.Fatal("expected diagnostic for invalid token")
	}
}
