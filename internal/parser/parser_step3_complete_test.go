...
func TestStep3AssignmentAndConditional(t *testing.T) {
	...
	if _, ok := assignment.Right.(ast.ConditionalExpr); !ok {
		t.Fatalf("expected conditional right side, got %#v", assignment.Right)
	}
}
...
func TestStep3AllAssignmentOperators(t *testing.T) {
	...
	for _, operator := range operators {
		...
		if len(p.Diagnostics()) != 0 {
			t.Fatalf("operator %q produced diagnostics: %v", operator, p.Diagnostics())
		}
		expr := file.Statements[0].(ast.ExprStmt).Expression
		assignment, ok := expr.(ast.AssignmentExpr)
		if !ok || assignment.Operator != operator {
			t.Fatalf("operator %q parsed as %#v", operator, expr)
		}
	}
}
...
func TestStep3RightAssociativeOperators(t *testing.T) {
	...
	if root.Operator != "**" {
		t.Fatalf("unexpected root operator: %s", root.Operator)
	}
	if _, ok := root.Right.(ast.BinaryExpr); !ok {
		t.Fatalf("expected right associative exponentiation, got %#v", root.Right)
	}
}
...
func TestStep3RightAssociativeAssignment(t *testing.T) {
	...
	if _, ok := root.Right.(ast.AssignmentExpr); !ok {
		t.Fatalf("expected right associative assignment, got %#v", root.Right)
	}
}
...
func TestStep3NestedPostfixExpressions(t *testing.T) {
	...
	if !ok || len(call.Arguments) != 1 {
		t.Fatalf("expected call with one argument, got %#v", file.Statements[0])
	}
	member, ok := call.Callee.(ast.MemberExpr)
	if !ok || member.Name != "name" || member.Optional {
		t.Fatalf("expected required member access, got %#v", call.Callee)
	}
	if _, ok := member.Object.(ast.IndexExpr); !ok {
		t.Fatalf("expected index before member, got %#v", member.Object)
	}
}
...
func TestStep3OptionalMemberAccess(t *testing.T) {
	...
	if root.Name != "name" || root.Optional {
		t.Fatalf("unexpected root member: %#v", root)
	}
	optional, ok := root.Object.(ast.MemberExpr)
	if !ok || !optional.Optional || optional.Name != "profile" {
		t.Fatalf("expected optional profile access, got %#v", root.Object)
	}
}
...
func TestStep3AssignmentTargetDiagnostic(t *testing.T) {
	...
	if len(p.Diagnostics()) == 0 || p.Diagnostics()[0].Code != "G2028" {
		t.Fatalf("expected G2028, got %v", p.Diagnostics())
	}
}
...
func TestStep3MalformedCollectionsRecover(t *testing.T) {
	...
	if len(p.Diagnostics()) == 0 {
		t.Fatal("expected diagnostics for malformed array")
	}
	if len(file.Statements) < 1 {
		t.Fatal("parser should retain recoverable statements")
	}
}
...
func TestStep3MalformedObjectRecoversToNextProperty(t *testing.T) {
	...
	if len(p.Diagnostics()) == 0 {
		t.Fatal("expected diagnostics for malformed object property")
	}
	if len(file.Statements) != 1 {
		t.Fatalf("expected one recoverable statement, got %d", len(file.Statements))
	}
	...
	if !ok || len(obj.Properties) != 1 || obj.Properties[0].Key != "age" {
		t.Fatalf("expected recovery to preserve age property, got %#v", decl.Value)
	}
}

func TestStep3StrayClosingBraceDoesNotHang(t *testing.T) {
	p, file := parseStep3Complete(`} create variable good as 3`)
	if len(p.Diagnostics()) == 0 || p.Diagnostics()[0].Code != "G2034" {
		t.Fatalf("expected G2034, got %v", p.Diagnostics())
	}
	if len(file.Statements) != 1 {
		t.Fatalf("expected parser to continue after stray brace, got %d statements", len(file.Statements))
	}
}
...
func TestStep3FunctionTrailingComma(t *testing.T) {
	...
	if !ok || len(fn.Parameters) != 2 {
		t.Fatalf("unexpected function: %#v", file.Statements[0])
	}
}
...
func TestStep3ObjectAndArrayTrailingCommas(t *testing.T) {
	...
	if !ok || len(obj.Properties) != 2 {
		t.Fatalf("unexpected object: %#v", decl.Value)
	}
	array, ok := obj.Properties[1].Value.(ast.ArrayExpr)
	if !ok || len(array.Items) != 2 {
		t.Fatalf("unexpected array: %#v", obj.Properties[1].Value)
	}
}
