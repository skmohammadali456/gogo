...
func TestParseFunctionIfElse(t *testing.T) {
	...
	if !ok || len(fn.Parameters) != 1 { t.Fatalf("unexpected function AST: %#v", file.Statements[0]) }
	if len(fn.Body.Statements) != 1 { t.Fatalf("unexpected function body: %#v", fn.Body) }
	if _, ok := fn.Body.Statements[0].(ast.IfStmt); !ok { t.Fatalf("expected if statement") }
}
...
func TestParseArraysObjectsMembersAndIndexes(t *testing.T) {
	...
	if !ok || member.Name != "name" { t.Fatalf("unexpected member AST: %#v", file.Statements[0]) }
	if _, ok := member.Object.(ast.IndexExpr); !ok { t.Fatalf("expected index before member") }

	p, file = parseStep3(`[1, 2, 3]`)
	if len(p.Diagnostics()) != 0 { t.Fatalf("unexpected array diagnostics: %v", p.Diagnostics()) }
...
func TestParseMultilingualFunctionNamesAndParameters(t *testing.T) {
	...
	if fn.Name.Name != "অভিবাদন" || fn.Parameters[0].Name != "नाम" { t.Fatalf("unexpected multilingual AST: %#v", fn) }
}
...
func TestParseRecoveryAfterInvalidStatement(t *testing.T) {
	...
	if len(p.Diagnostics()) == 0 { t.Fatal("expected parser diagnostics") }
	if len(file.Statements) == 0 { t.Fatal("parser should recover and continue") }
}
